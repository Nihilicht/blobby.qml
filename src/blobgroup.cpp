...
void BlobGroup::setBorderCompression(qreal v) {
    if (qFuzzyCompare(m_borderCompression, v))
        return;
    m_borderCompression = v;
    emit borderCompressionChanged();
    markDirty();
}

void BlobGroup::setMinFlattenRadius(qreal v) {
    if (qFuzzyCompare(m_minFlattenRadius, v))
        return;
    m_minFlattenRadius = v;
    emit minFlattenRadiusChanged();
    markDirty();
}
