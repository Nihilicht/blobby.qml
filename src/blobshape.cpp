...
void BlobShape::setPhysicsEnabled(bool enabled) {
    if (m_physicsEnabled == enabled)
        return;
    m_physicsEnabled = enabled;
    emit physicsEnabledChanged();
}
...
void BlobShape::updatePolish() {
    ...
    const float minR = static_cast<float>(m_group->minFlattenRadius());
...
QSGNode* BlobShape::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData*) {
    ...
    material->m_borderCompression = static_cast<float>(m_group->borderCompression());
